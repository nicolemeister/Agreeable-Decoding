...
for i in range(BATCH_SIZE):
    print(i)
    result_gen = summarization_decoding(args, questions[int(i*len(questions)/BATCH_SIZE): int((i+1)*len(questions)/BATCH_SIZE)], prompt_dem, prompt_repub, N=args.N)
...
# EVAL
result_eval = eval(args, questions, prompt_dem, prompt_repub, result_gen=result_gen)
for question in questions: result_eval[question]['data'] = result_eval[question]['data'].tolist() # convert to json serializable

out_file = open("{}/{}/00{}/{}/{}/temp_{}/score_N_3.json".format(args.model_output_dir, args.input_data, args.model_num, args.prompt_strength,args.question_type, args.temp), "w")
