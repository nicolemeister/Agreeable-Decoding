...
def get_data(polarizingqs_path, polarizingqs_large_path, triviaQAmini_path, triviaQA_path=None):
    # triviaQAmini_path
    ...
    trivia_offdiaggap_polarizing_dict = {}
    ...
    for question in questions:
        if question != 'avg':
            if not math.isnan(data[question]['off_diagonal_gap']):
                ...
                trivia_offdiaggap_polarizing_dict[question]=data[question]['off_diagonal_gap']

    # polarizingqs_large_path
    with open(polarizingqs_large_path, 'r') as f:
        data = json.load(f)
    questions = data.keys()
    for question in questions:
        if question != 'avg':
    ...
    return trivia_offdiaggap_polarizing, trivia_offdiaggap, trivia_offdiaggap_polarizing_dict

def optimal_threshold(polarizingqs_path, polarizingqs_large_path, triviaQAmini_path, triviaQA_path=None):

    trivia_offdiaggap_polarizing, trivia_offdiaggap, trivia_offdiaggap_polarizing_dict  = get_data(polarizingqs_path, polarizingqs_large_path, triviaQAmini_path, triviaQA_path)
    thresholds = []
    ...
    return optimal_threshold, trivia_offdiaggap_polarizing, trivia_offdiaggap_polarizing_dict
